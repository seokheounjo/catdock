...
export function AgentSlot({ agent, status, recovering = false, dockSize = 'medium', onClick, onContextMenu }: Props) {
  const { t } = useI18n()
  ...
  return (
    ...
    >
      {/* 디렉터 배지 — 총괄 ★★ */}
      {isDirector && (
        <span className="text-yellow-400 text-[10px] leading-none mb-[-2px]" aria-hidden="true">★★</span>
      )}
      {/* 리더 왕관 */}
      {isLeader && (
        <span className="text-yellow-400 text-[10px] leading-none mb-[-2px]" aria-hidden="true">&#9733;</span>
      ...
      <div className="relative">
        <FishingCat status={status} recovering={recovering} size={(isDirector || isLeader) ? 'large' : 'normal'} dockSize={dockSize} />
        {/* 품종별 아바타 뱃지 */}
        <div className={`absolute -bottom-1 -right-1 rounded-full border-2 border-chat-bg overflow-hidden bg-white/10 ${
      ...
      <div className="flex flex-col items-center">
        <span className={`text-[9px] truncate leading-none ${
          isDirector ? 'text-yellow-300/80 max-w-[64px]' : isLeader ? 'text-yellow-300/80 max-w-[64px]' : 'text-text-secondary max-w-[56px]'
        }`}>{agent.name}</span>
        {/* 리더/디렉터 역할 표시 */}
        {(isDirector || isLeader) && agent.role && (
          <span className="text-[7px] text-blue-300/70 leading-none truncate max-w-[72px]">{agent.role}</span>
        )}
        {/* 복구 중 표시 */}
        {recovering && (
          <span className="text-[7px] text-amber-400/80 leading-none animate-pulse">{t('dock.statusRecovering')}</span>
        )}
        {/* 임시 에이전트 표시 */}
        {isTemporary && !recovering && (
          <span className="text-[7px] text-orange-400/60 leading-none">{t('agentCard.temporary')}</span>
        )}
        <span
          id={`agent-${agent.id}-status`}
          className="text-[8px] text-text-muted sr-only"
        >

...
export function Dock() {
  ...
  useEffect(() => {
    ...
    fetchStates()
    const unsubs = [
      window.api.on('agent:created', () => { fetchAgents(); fetchStates() }),
      window.api.on('agent:updated', () => { fetchAgents(); fetchStates() }),
      window.api.on('agent:deleted', () => { fetchAgents(); fetchStates() }),
      window.api.on('agent:status-changed', () => fetchStates()),
      window.api.on('agent:process-info-changed', () => fetchStates()),
      window.api.on('dock:size-changed', (size: unknown) => { setDockSize(size as DockSize); setDynamicGap(null) }),
  ...
  return (
    <>
      <div className="flex items-end gap-1 flex-nowrap overflow-visible pt-4"
        style={dynamicGap && dynamicGap < 60 ? { gap: `${Math.max(0, dynamicGap - 40)}px` } : undefined}
      >
        {agents.map(agent => {
          ...
          >
            ...
            <button
              ...
              onClick={async () => {
                ...
                const nextRole = currentRole === 'member' ? 'leader'
                  : currentRole === 'leader' ? 'director'
                  : 'member'
